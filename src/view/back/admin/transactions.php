    ...
    <div v-if="showMessagesModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" @click.self="closeMessagesModal">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-3xl w-full p-6 sm:p-8 max-h-[80vh] overflow-y-auto">
            ...
            <div v-else class="space-y-4">
                <div v-for="conversation in conversationsList" :key="conversation.user_id"
    ...
    createApp({
        ...
        methods: {
            ...
            async openMessagesModal(listingId) {
                ...
                const currentUserId = 1; // Admin ID - should ideally come from session

                messages.forEach(msg => {
                    // Skip if both sender and receiver are the same person (shouldn't happen)
                    if (msg.sender_id === msg.receiver_id) {
                        return;
                    }

                    // Create a unique conversation key based on the pair of users
                    // Always use the smaller ID first to ensure consistency
                    const user1 = Math.min(msg.sender_id, msg.receiver_id);
                    const user2 = Math.max(msg.sender_id, msg.receiver_id);
                    const conversationKey = `${listingId}_${user1}_${user2}`;

                    // Determine which user is the "other" user (not the current user)
                    let otherUserId, otherFirstName, otherLastName, otherEmail;

                    if (msg.sender_id === currentUserId) {
                        // Current user sent this message
                        otherUserId = msg.receiver_id;
                        otherFirstName = msg.receiver_first_name;
                        otherLastName = msg.receiver_last_name;
                        otherEmail = msg.receiver_email;
                    } else if (msg.receiver_id === currentUserId) {
                        // Current user received this message
                        otherUserId = msg.sender_id;
                        otherFirstName = msg.sender_first_name;
                        otherLastName = msg.sender_last_name;
                        otherEmail = msg.sender_email;
                    } else {
                        // This message doesn't involve the current user - skip it
                        return;
                    }
                    ...
                    if (!conversationsMap.has(conversationKey)) {
                        conversationsMap.set(conversationKey, {
                            ...
                            sender_first_name: otherFirstName, // Keep for chat modal
                            sender_last_name: otherLastName, // Keep for chat modal
                            email: otherEmail,
                            messages: [],
                            last_message: '',
                            last_message_date: '',
